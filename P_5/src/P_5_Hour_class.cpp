...
Hour::Hour_class::Hour_class(int h, int m, int s, const std::string& tf) : hours{h}, minutes{m}, seconds{s}, time_format{tf} {
...
Hour::Hour_class::Hour_class(const Hour_class& init_obj) : hours{init_obj.hours}, minutes{init_obj.minutes}, seconds{init_obj.seconds}, time_format{init_obj.time_format} {std::cout << "Copy constructor!\n";}
...
inline bool Hour::Hour_class::time_format_24_H(void) const {
  return this->time_format == "24 HOURS";
}

bool Hour::Hour_class::is_time_correct(void) const {
...
bool Hour::Hour_class::set_time(int h, int m, int s, std::string& format_string) {
...
void Hour::Hour_class::get_time(int& target_hours, int& target_minutes, int& target_seconds, std::string& target_format) const {
  target_hours = this->hours; target_minutes = this->minutes; target_seconds = this->seconds; target_format = this->time_format;
}

Hour::Hour_class::~Hour_class() {
  std::cout << "Destructor!\n";
}
