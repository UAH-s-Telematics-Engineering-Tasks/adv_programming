...
#include "../inc/P_8_a_Enterprise_class.h"
#include "../inc/Utils.h"
...
int main() {
    while(true) {
      switch(utils::General_utils::show_menu(menu, sizeof(menu) / sizeof(char*))) {
          case 1:
        {
          List_class<std::string> test_list;
          std::string names[] = {"Name_1", "Name_2", "Name_3", "Name_4", "Name_5",
            "Name_6", "Name_7", "Name_8", "Name_9", "Name_10"};

          for (int i = 0; i < 10; i++)
            test_list.add_element(names[i]);

          while (test_list.is_there_more()) {
            try {
              std::cout << test_list.get_curr() << '\n';
            }
            catch (incorrect_index_ex e) {
              std::cout << e.what() << '\n';
            }
          ...
          if (test_list.am_i_empty())
            std::cout << "I'm empty!\n";
          else
            std::cout << "Error freeing the list...\n";

          std::cout << "Answer 1: No, it's not necessary as we can access any pointer to a Node we need through the methods we have defined in List_class itself!\n";
          getchar();
          break;
        }
      case 2:
        {
          List_class<int> i_list;
          int acc = 0;

          for (int i = 0; i < 10; i++)
            i_list.add_element(i);
